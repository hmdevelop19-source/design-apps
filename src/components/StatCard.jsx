...
export default function StatCard({ label, value, desc, icon: Icon, iconColor, isDarkVariant, badge, badgeColor }) {
  return (
    <div className={`p-6 rounded-3xl ${isDarkVariant ? 'bg-gradient-to-br from-[#000052] to-[#000075] text-white' : 'bg-bg-card dark:bg-bg-cardDark text-text-main dark:text-text-dark'} shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] hover:-translate-y-1.5 transition-all duration-500 relative group overflow-hidden`}>
      ...
      {badge && (
        <span 
          ...
          style={{ backgroundColor: isDarkVariant ? 'var(--color-accent)' : `${badgeColor}20`, color: isDarkVariant ? '#fff' : badgeColor, ...(isDarkVariant ? { backgroundColor: '#FCD526' } : {}) }}
        >
          {badge}
        </span>
      )}
      ...
      <div 
        ...
        style={{ backgroundColor: isDarkVariant ? 'rgba(255,255,255,0.1)' : `${iconColor}15`, color: isDarkVariant ? '#FCD526' : iconColor }}
      >
